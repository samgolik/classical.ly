...
class _OnboardingFlowState extends State<OnboardingFlow> {
  ...
  @override Widget build(BuildContext context) => Scaffold(body: SafeArea(child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [
    ...
    const Spacer(), Expanded(flex: 6, child: PageView(controller: controller, onPageChanged: (value) => setState(() => page = value), children: [
      const _Welcome(),
      _ProfileSetup(name: name, instrument: instrument),
    ])),
    Row(children: List.generate(2, (index) => AnimatedContainer(duration: const Duration(milliseconds: 180), margin: const EdgeInsets.only(right: 7), height: 6, width: page == index ? 28 : 6, decoration: BoxDecoration(color: page == index ? AppTheme.gold : AppTheme.cream.withValues(alpha: .2), borderRadius: BorderRadius.circular(10)))), const Spacer(),
    FilledButton(onPressed: page == 0 ? () => controller.nextPage(duration: const Duration(milliseconds: 280), curve: Curves.easeOutCubic) : finish, child: Text(page == 0 ? 'Begin your archive' : 'Create my profile')), const SizedBox(height: 12),
  ])));
}
