...
class _LogPageState extends State<LogPage> {
  ...
  void save() {
    if (title.text.trim().isEmpty || composer.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Add a work title and composer first.'))); return; }
    final work = widget.preselected ?? Work(id: DateTime.now().microsecondsSinceEpoch.toString(), title: title.text.trim(), composer: composer.text.trim(), subtitle: subtitle.text.trim(), difficulty: 0, duration: '', instrumentation: const []);
    RepertoireStore.instance.addLog(PerformanceLog(work: work, ensemble: ensemble.text.trim(), date: DateTime.now(), role: role.text.trim(), note: note.text.trim(), rating: rating));
